...
// draw bound.
extern "C"
int gr_node_drawbound_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  if (self->m_nodeType == NodeType::GeometryNode)
  {
    if (((GeometryNode*)self)->m_primitive->getType() == PrimitiveType::PrimitiveMesh)
    {
      Mesh *themesh = (Mesh*)(((GeometryNode*)self)->m_primitive);
      themesh->drawBound();
    }
  }
...
// draw mesh.
extern "C"
int gr_node_drawmesh_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
// set AA size
extern "C"
int gr_node_set_AA_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  
  gr_node_ud* selfdata = (gr_node_ud*)luaL_checkudata(L, 1, "gr.node");
  size_t aasize = luaL_checknumber(L, 2);
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"drawbound", gr_node_drawbound_cmd},
  {"drawmesh", gr_node_drawmesh_cmd},
  {"set_AA", gr_node_set_AA_cmd},
  {0, 0}
};
